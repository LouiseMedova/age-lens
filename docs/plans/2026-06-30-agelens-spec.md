...
Note: this was the initial v0.1 query-only spec. VAN project review `6` requested an auditable receipt layer; the deployable v0.2 design is specified in `2026-06-30-agelens-receipts-spec.md`.
...
- Storage of user profiles or open-ended eligibility history.
- Receipt storage for deployable v0.2 is covered in `2026-06-30-agelens-receipts-spec.md`.
...
- AgeLens program: validates dates and returns deterministic derived facts. In v0.2, callers can explicitly record auditable receipts for downstream verification.

## State Changes
Superseded for deploy: v0.2 owns receipt state keyed by `calculation_id`.
The original query methods still do not persist input dates or results unless
the caller explicitly uses `RecordCalculation`.
...
## Events
Superseded for deploy: v0.2 emits `CalculationRecorded` only when a caller
explicitly records a receipt. Plain query methods still do not emit events.

## Invariants
- Plain query methods do not write input dates to contract state.
- Receipt events exist only for explicit `RecordCalculation` calls.
...
- The generated IDL exposes the original query methods plus the v0.2 receipt methods documented in `2026-06-30-agelens-receipts-spec.md`.
